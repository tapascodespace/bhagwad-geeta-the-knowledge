...
Deno.serve(async (req) => {
  ...
  try {
    const apiKey = Deno.env.get("ELEVENLABS_API_KEY_ACCOUNT2");
    if (!apiKey) {
      console.error("tts-account2: ELEVENLABS_API_KEY not configured");
    ...
    if (!res.ok) {
      ...
      console.error(`tts-account2 ElevenLabs ${res.status}:`, errText);
      return new Response(
    ...
    console.error("tts-account2 error:", e);
    return new Response(JSON.stringify({ error: "Service unavailable" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
